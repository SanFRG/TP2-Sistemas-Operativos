...
void load_idt(void) {
  ...
  // Sobreescribir con handlers específicos que sí usamos
  ...
  setup_IDT_entry (0x80, (uint64_t)&_irq80Handler);        // Syscall (DPL=3)

  // Enmascarar/Desenmascarar PIC según corresponda (timer/teclado habilitados)
  picMasterMask(0xFC);
  picSlaveMask(0xFF);
